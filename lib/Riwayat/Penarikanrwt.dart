...
import '/Riwayat/rentangwaktu.dart';
...
class Penarikanrwt extends StatefulWidget {
  ...
  @override
  State<Penarikanrwt> createState() => _PenarikanrwtState();
}
...
class _PenarikanrwtState extends State<Penarikanrwt> {
  ...
  Future<void> getData({
    ...
    if (token.isNotEmpty) {
      ...
      final response = await DioProvider().getUserHistoryPenarikan(
        token,
  ...
  Future<void> downloadTransactionData({
    ...
    try {
      ...
      final downloadedFilePath = await DioProvider().downloadPenarikanPDF(
        token,
        filterType: filterType,
        selectedMonth: selectedMonth,
        selectedYear: selectedYear,
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 1),
            (data['penarikan'] != null && data['penarikan'] is List)
                ? Expanded(
                    child: ListView.builder(
                    itemCount: data['penarikan'].length,
                    itemBuilder: (context, index) {
                      final transaction = data['penarikan'][index];
                      ...
                      final previousDate = index > 0
                          ? formatDateTime(
                              data['penarikan'][index - 1]['updated_at'])
                          : '';
                ...
                : const Center(
                    child: Text('Tidak ada transaksi yang ditemukan.', style: TextStyle(color: Color(0xFF7B5233)),),
                  ),
          ],
        ),
      ),
